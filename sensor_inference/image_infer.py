...
def build_image_engine(onnx_file, trt_file):
...
class ImageInfer(InferBase):
    def __init__(self, engine_start, cfg_file = None, logger = None, max_size = 3):
        super().__init__('imageDet', engine_start, cfg_file, logger, max_size)
    ...
    def build_engine(self, calib):
        from sensor_inference.utils.image_post_process import ImagePostProcesser
        self.engine = build_image_engine(self.cfg.ONNX_FILE, self.cfg.TRT_FILE)
